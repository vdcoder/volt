...
def create_app(name, guid, destination, no_git):
    ...
    shutil.copytree(repo / 'framework/include', destination / 'dependencies/volt/include')
    shutil.copytree(repo / 'framework/src', destination / 'dependencies/volt/src')
    ...
    tokens = {
        ...
        'VOLT_APP_NAME': name,
        'VOLT_CLIENT_PROJECT_GUID': str(uuid.uuid4()).upper(),
        'VOLT_SERVER_PROJECT_GUID': str(uuid.uuid4()).upper(),
    }
    # Only our template files are substituted, never dependency sources/binaries.
    files = list((destination / 'client').rglob('*')) + list((destination / 'server').glob('*.vcxproj'))
    ...
    (destination / 'VOLT_APP_NAME.sln').rename(destination / f'{name}.sln')
...
if __name__ == '__main__':
    ...
    try:
        create_app(args.name, args.guid or args.name, args.output, args.no_git)
